...
def resultados():
    usuarios = []
    with open('usuarios_examen.csv', newline='', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            usuarios.append(row)
    return render_template('repo.html', usuarios=usuarios)
...
def enlaces():
    ...
    return render_template("enlace.html", archivos=archivos_pdf)
